...
for i=1:itt
    ...
    [x,y, confi] = OneScaleObjectDetection( x, y, Scale, IntegralImages, w, h, HaarCasade);
     
    % If search coordinates still exist, they contain an Object
    for k=1:length(x);
        n=n+1; Objects(n,:)=[x(k) y(k) w h confi(k)];
    end
end
...
Objects(:,1:4)=Objects(:,1:4)*IntegralImages.Ratio;