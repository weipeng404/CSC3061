function [x,y, confi]=OneScaleObjectDetection( x, y, Scale, IntegralImages, w,h,HaarCasade)
...
for i_stage = 1:length(HaarCasade.stages),
    ...
    %Confidence in its own detection is based on how much bigger than the
    %minimum threshold is each of the detections
    confi = confi+(StageSum-stage.stage_threshold)/length(HaarCasade.stages);
